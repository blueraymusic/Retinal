...
def train_model_v2(model, criterion, optimizer, scheduler, model_name=None, weights=None, num_epochs=25):
    model_name = model_name if model_name else model.__class__.__name__
    since = time.time()
    # weights = weights.to(device)
    best_model_wts = copy.deepcopy(model.state_dict())
...
if __name__ == '__main__':
    ...
    model_ft, loss_history = train_model(model_ft, criterion, optimizer, exp_lr_scheduler,
                                      num_epochs=epochs, model_name='ResNet50')

    # model, train_loss, validation_loss = train_model_v2(model, criterion, optimizer, exp_lr_scheduler,
    #                                                     num_epochs=epochs, model_name='ResNet50')

    print_loss_history(loss_history['train'], loss_history['val'])
